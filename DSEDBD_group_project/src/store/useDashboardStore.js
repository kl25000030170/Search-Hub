  ...
  fetchDashboardData: async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to fetch admin stats from Node.js:", err);
      }

      const data = await dashboardApi.fetchDashboardAnalytics().catch(() => null);

      set({
        analytics: {
          totalItems: data?.totalItems ?? get().analytics.totalItems,
          totalCategories: data?.totalCategories ?? get().analytics.totalCategories,
          searchCount: stats?.totalSearches ?? data?.searchCount ?? data?.totalSearches ?? get().analytics.searchCount,
          topFilterUsage: data?.topFilterUsage ?? get().analytics.topFilterUsage,
          popularCategories: (stats?.popularCategories && stats.popularCategories.length > 0) 
            ? stats.popularCategories 
            : (data?.popularCategories ?? data?.trendingCategories ?? get().analytics.popularCategories),
          recentQueries: (stats?.topTrendingSearches && stats.topTrendingSearches.length > 0)
            ? stats.topTrendingSearches
            : (data?.recentQueries ?? get().analytics.recentQueries),
          searchFrequency: (stats?.searchTrends && stats.searchTrends.length > 0)
            ? stats.searchTrends.map(trend => ({ name: trend.day, searches: trend.searches }))
            : (data?.searchFrequency ?? get().analytics.searchFrequency),
        },
        loading: false,
      });
      return;
