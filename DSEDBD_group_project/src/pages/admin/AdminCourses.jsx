...
const AdminCourses = () => {
  ...
  const openAdd = () => {
    setEditingId(null);
    setForm({ ...emptyCourse, category: categoryNames.includes('Courses') ? 'Courses' : (categoryNames[0] || 'Courses') });
    setModalOpen(true);
  };
  ...
  const handleSave = (e) => {
    ...
    const payload = {
      ...
      rating: 4.5,
      instructor: 'Platform Admin', // static fallback for user dashboard compatibility
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button type="button" onClick={openAdd} className="btn-primary flex items-center gap-2 text-sm px-5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-border text-left">
                ...
                <th className="px-5 py-4 font-bold text-muted-foreground">Difficulty</th>
            ...
            <tbody>
              {courses.map((c) => (
                <tr key={c.id} className="border-b border-border/50 hover:bg-secondary/20">
                  ...
                  <td className="px-5 py-3">
                    <span className="text-xs font-bold px-2 py-1 rounded-full bg-violet-500/10 text-violet-600">{c.level || 'Beginner'}</span>
                  </td>
                  <td className="px-5 py-3">
