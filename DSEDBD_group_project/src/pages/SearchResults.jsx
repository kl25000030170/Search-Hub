...
export const SearchResults = () => {
  ...
  useEffect(() => {
    fetchResults();
  }, [query, isSemantic, filters, fetchResults]);
                  ...
                  {Object.entries(groupedResults).map(([category, items]) => (
                    <div key={category} className="space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
                        {items.map((item) => (
                          <ItemResultCard key={item.id} item={item} isSemantic={isSemantic} onView={() => navigate(`/items/${item.id}`)} />
                        ))}
                      </div>
                    </div>
                  ))}
...
const ItemResultCard = ({ item, isSemantic, onView }) => {
  return (
    <motion.div
      layout
      ...
      className={clsx(
        "group bg-card border rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full",
        isSemantic && item.matchScore >= 95 
          ? "border-indigo-500/30 ring-2 ring-indigo-500/5 bg-gradient-to-b from-indigo-500/[0.02] to-card" 
          : "border-border"
      )}
    >
      {/* Thumbnail layout */}
