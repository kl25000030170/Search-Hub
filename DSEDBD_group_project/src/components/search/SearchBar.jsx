...
export const SearchBar = ({ onSearch }) => {
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      try {
        const res = await analyticsApi.getSuggestions(query);
        const data = res?.data || res || [];
        if (Array.isArray(data) && data.length > 0) {
          setSuggestions(data);
        } else {
          // Fallback to local filter if no results from backend
        ...
        console.error('Failed to fetch live suggestions, using fallback:', err);
        const filtered = MOCK_AUTO_SUGGESTIONS.filter((s) =>
          s.toLowerCase().includes(query.toLowerCase())
        );
        setSuggestions(filtered);
